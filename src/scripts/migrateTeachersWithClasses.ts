...
async function migrateTeachersWithClasses(options: MigrationOptions = {}) {
  ...
  try {
    ...
    for (const teacher of teachers) {
      try {
        console.log(`\n👨‍🏫 Processing teacher: ${(teacher.userId as any)?.name || teacher._id}`);
    ...
    if (teachersWithClassIds.length > 0) {
      ...
      sampleTeachers.forEach((teacher, index) => {
        ...
        console.log(`   Subjects (${teacher.subjectIds.length}): ${teacher.subjectIds.map((s: any) => `${s.code} (L${s.level})`).join(', ')}`);
        console.log(`   Classes (${teacher.classIds.length}): ${teacher.classIds.map((c: any) => `${c.name} (L${c.level})`).join(', ')}`);
      });
    }
