...
async function migrateAdminIsolation() {
    try {
        ...
        const updates = [];
        // Update Students
        const studentResult = await Student.updateMany({ adminId: { $exists: false } }, { $set: { adminId: superAdmin._id } });
        console.log(`✅ Updated ${studentResult.modifiedCount} students with adminId`);
        // Update Teachers
        const teacherResult = await Teacher.updateMany({ adminId: { $exists: false } }, { $set: { adminId: superAdmin._id } });
        console.log(`✅ Updated ${teacherResult.modifiedCount} teachers with adminId`);
        // Update Classes
        const classResult = await Class.updateMany({ adminId: { $exists: false } }, { $set: { adminId: superAdmin._id } });
        console.log(`✅ Updated ${classResult.modifiedCount} classes with adminId`);
        // Update Subjects
        const subjectResult = await Subject.updateMany({ adminId: { $exists: false } }, { $set: { adminId: superAdmin._id } });
        console.log(`✅ Updated ${subjectResult.modifiedCount} subjects with adminId`);
        // Update Questions
        const questionResult = await Question.updateMany({ adminId: { $exists: false } }, { $set: { adminId: superAdmin._id } });
        console.log(`✅ Updated ${questionResult.modifiedCount} questions with adminId`);
        // Update Exams
        const examResult = await Exam.updateMany({ adminId: { $exists: false } }, { $set: { adminId: superAdmin._id } });
        console.log(`✅ Updated ${examResult.modifiedCount} exams with adminId`);
        // Update Syllabi
        const syllabusResult = await Syllabus.updateMany({ adminId: { $exists: false } }, { $set: { adminId: superAdmin._id } });
...
//# sourceMappingURL=migrateAdminIsolation.js.map