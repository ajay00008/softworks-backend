...
async function run() {
    ...
    try {
        ...
        for (const teacherInfo of teacherData) {
            const existingTeacher = await User.findOne({ email: teacherInfo.email });
            if (!existingTeacher) {
        ...
        for (const studentInfo of studentData) {
            const existingStudent = await User.findOne({ email: studentInfo.email });
            if (!existingStudent) {
        ...
        for (const teacher of createdTeachers) {
            ...
            console.log(`${teacher.name}: ${teacherSubjects.map(s => s.shortName).join(', ')}`);
        }
...
//# sourceMappingURL=seedCompleteSampleData.js.map