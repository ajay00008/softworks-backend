...
export class AIAnswerCheckerService {
     ...
     */
    async checkAnswerSheet(answerSheetId) {
     ...
     */
    async processAnswerSheet(answerSheet, exam) {
     ...
     */
    async analyzeQuestion(answerSheet, question, questionNumber) {
     ...
     */
    async extractStudentAnswer(answerSheet, questionNumber) {
     ...
     */
    async checkCorrectness(studentAnswer, correctAnswer, question) {
     ...
     */
    calculatePartialMarks(studentAnswer, correctAnswer, question, maxMarks) {
        const similarity = this.calculateSemanticSimilarity(studentAnswer, correctAnswer);
     ...
     */
    calculateSemanticSimilarity(answer1, answer2) {
        // Simulate AI semantic analysis
     ...
     */
    calculateQuestionConfidence(studentAnswer, correctAnswer, question) {
        const similarity = this.calculateSemanticSimilarity(studentAnswer, correctAnswer);
     ...
     */
    calculateOverallConfidence(questionResults) {
     ...
     * Generate comprehensive feedback
     */
    generateComprehensiveFeedback(questionResults, percentage) {
     ...
     */
    generateQuestionFeedback(studentAnswer, correctAnswer, question, isCorrect) {
        if (isCorrect) {
     ...
     */
    assessHandwritingQuality(answer) {
     ...
     */
    analyzeHandwriting(answerSheet) {
        return {
     ...
     */
    generateAcademicInsights(questionResults, percentage) {
     ...
     */
    async updateAnswerSheetWithResults(answerSheetId, result) {
     ...
     */
    async batchCheckAnswerSheets(answerSheetIds) {
        const results = [];
        ...
        if (answerSheets.length > 0) {
            ...
            percentages.forEach(percentage => {
                if (percentage >= 85)
                    ...
                    stats.qualityDistribution.average++;
                else
                    stats.qualityDistribution.needsImprovement++;
            });
        }
        return stats;
    }
}
//# sourceMappingURL=aiAnswerChecker.js.map