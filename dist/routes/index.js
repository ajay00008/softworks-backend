...
import { createSubject, getSubjects, getSubject, updateSubject, deleteSubject, getSubjectsByCategory, getSubjectsByLevel, uploadReferenceBook, uploadReferenceBookToSubject, uploadReferenceBookBase64, downloadReferenceBook, checkReferenceBookExists, deleteReferenceBook } from "../controllers/subjectController";
...
import { getTeacherAccess, createTeacherQuestionPaper, uploadAnswerSheets, markStudentStatus, evaluateAnswerSheets, getResults, getPerformanceGraphs, getTeacherExams, downloadResults, getAnswerSheets } from "../controllers/teacherDashboardController";
import { createStaffAccess, getAllStaffAccess, getStaffAccess, updateStaffAccess, deleteStaffAccess } from "../controllers/staffAccessController";
...
import { createSamplePaper, getSamplePapers, getSamplePaperById, updateSamplePaper, deleteSamplePaper, downloadSamplePaper, analyzeSamplePaper, uploadSamplePaper } from "../controllers/samplePaperController";
const router = Router();
...
import { checkAnswerSheetWithAI, batchCheckAnswerSheetsWithAI, getAIResults, getAIStats, overrideAIResult, getAnswerSheetsForAIChecking, recheckAnswerSheetWithAI } from "../controllers/aiAnswerCheckerController";
import { validateAIRequest, validateOverrideRequest, validatePagination, rateLimitAI, validateAnswerSheetUpload, handleAIError } from "../middleware/aiValidation";
/**
 * @openapi
 ...
 *         description: Unauthorized
 */
router.get("/teacher/notifications", requireAuth, requireRoles("TEACHER"), async (req, res) => {
    try {
        const userId = req.auth?.sub;
        const { limit = 20, offset = 0 } = req.query;
        const { NotificationService } = await import('../services/notificationService');
        const notifications = await NotificationService.getNotifications(userId, Number(limit), Number(offset));
        res.json({
            success: true,
            data: notifications
        });
 ...
 */
router.post("/teacher/notifications/:notificationId/read", requireAuth, requireRoles("TEACHER"), async (req, res) => {
    try {
        const userId = req.auth?.sub;
 ...
 */
router.post("/teacher/notifications/read-all", requireAuth, requireRoles("TEACHER"), async (req, res) => {
    try {
        const userId = req.auth?.sub;
        const { NotificationService } = await import('../services/notificationService');
