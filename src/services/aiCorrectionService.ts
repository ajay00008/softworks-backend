...
export class AICorrectionService {
   ...
   */
  private static createCorrectionPrompt(request: AICorrectionRequest): string {
...
- Focus on learning outcomes and improvement`;
   ...
   */
  private static parseCorrectionResponse(response: string, request: AICorrectionRequest): Omit<AICorrectionResult, 'processingTime'> {
    try {
      ...
      return {
        ...
        errors: [`Failed to parse AI response: ${(error as Error).message}`]
      };
    }
  }
   ...
   */
  static async generateMockCorrection(request: AICorrectionRequest): Promise<AICorrectionResult> {
