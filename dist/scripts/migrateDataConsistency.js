...
async function migrateDataConsistency() {
    try {
        ...
        for (const teacher of teachersWithSubjects) {
            const validSubjects = teacher.subjectIds.filter((subject) => subject !== null);
            if (validSubjects.length !== teacher.subjectIds.length) {
                ...
                await Teacher.findByIdAndUpdate(teacher._id, {
                    subjectIds: validSubjects.map((subject) => subject._id)
                });
            }
        }
        ...
        for (const classItem of classes) {
            ...
            const availableSubjects = subjects.filter(subject => subject.level.includes(classLevel));
...
//# sourceMappingURL=migrateDataConsistency.js.map