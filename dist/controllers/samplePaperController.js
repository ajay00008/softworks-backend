...
// Create Sample Paper
export async function createSamplePaper(req, res, next) {
    try {
...
export async function getSamplePapers(req, res, next) {
    try {
        const auth = req.auth;
        const adminId = auth?.adminId;
        if (!adminId) {
...
export async function getSamplePaperById(req, res, next) {
    try {
        const { id } = req.params;
        const auth = req.auth;
        const adminId = auth?.adminId;
...
// Update Sample Paper
export async function updateSamplePaper(req, res, next) {
    try {
        const { id } = req.params;
        ...
        const samplePaper = await SamplePaper.findOneAndUpdate({ _id: id, adminId, isActive: true }, updateData, { new: true })
            .populate('subjectId', 'code name shortName')
            .populate('uploadedBy', 'name email');
        if (!samplePaper) {
            throw new createHttpError.NotFound("Sample paper not found");
...
// Delete Sample Paper
export async function deleteSamplePaper(req, res, next) {
    try {
        const { id } = req.params;
        const auth = req.auth;
        const adminId = auth?.adminId;
...
export async function downloadSamplePaper(req, res, next) {
    try {
        const { id } = req.params;
        const auth = req.auth;
        const adminId = auth?.adminId;
...
export async function analyzeSamplePaper(req, res, next) {
    try {
        const { id } = req.params;
        const auth = req.auth;
        const adminId = auth?.adminId;
...
//# sourceMappingURL=samplePaperController.js.map