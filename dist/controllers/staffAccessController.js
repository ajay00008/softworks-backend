...
// Create staff access
export const createStaffAccess = async (req, res) => {
    try {
...
export const updateStaffAccess = async (req, res) => {
    try {
        ...
        if (classAccess)
            staffAccess.classAccess = classAccess;
        if (subjectAccess)
            staffAccess.subjectAccess = subjectAccess;
        if (globalPermissions)
            staffAccess.globalPermissions = globalPermissions;
        if (expiresAt !== undefined)
            staffAccess.expiresAt = expiresAt ? new Date(expiresAt) : undefined;
        if (notes !== undefined)
            staffAccess.notes = notes;
        await staffAccess.save();
...
export const getAllStaffAccess = async (req, res) => {
    try {
...
export const deactivateStaffAccess = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.auth?.sub;
        if (!userId) {
...
export const checkClassAccess = async (req, res) => {
    try {
...
export const checkSubjectAccess = async (req, res) => {
    try {
...
// Delete staff access
export const deleteStaffAccess = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.auth?.sub;
        if (!userId) {
...
//# sourceMappingURL=staffAccessController.js.map