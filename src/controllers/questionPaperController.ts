...
export async function generateQuestionPaper(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const questions = generatedQuestions.map((aiQuestion, index) => {
      ...
      const questionType = getQuestionTypeFromDistribution(
        finalSettings.questionTypeDistribution,
        index,
        finalSettings.questionTypeDistribution.length
      );
...
// Helper functions
function getQuestionTypeFromDistribution(distribution: any[], index: number, total: number): string {
...
function enhanceQuestionForType(questionText: string, questionType: string, gradeLevel: string): string {
  switch (questionType) {
    case 'FILL_BLANKS':
...
function getMarksForQuestionType(questionType: string, markDistribution: any[]): number {
  ...
  return typeMarks[questionType as keyof typeof typeMarks] || 2;
}

function getTimeLimitForGrade(gradeLevel: string): number {
  const timeLimits = {
    'PRE_KG': 2,
    'LKG': 3,
    'UKG': 5,
  ...
  return timeLimits[gradeLevel as keyof typeof timeLimits] || 5;
}

function generateMatchingPairs(questionText: string): { left: string; right: string }[] {
...
function generateDrawingInstructions(questionText: string, gradeLevel: string): string {
  const instructions = {
  ...
  return instructions[gradeLevel as keyof typeof instructions] || 'Draw the diagram as described';
}

function generateMarkingInstructions(questionText: string, gradeLevel: string): string {
  return `Mark the following parts on the diagram: 1) Main structure, 2) Supporting elements, 3) Key features`;
}

function generateVisualAids(questionType: string): string[] {
  const aids = {
  ...
  return aids[questionType as keyof typeof aids] || [];
}

function generateInteractiveElements(questionType: string): string[] {
  const elements = {
  ...
  return elements[questionType as keyof typeof elements] || [];
}

function getQuestionTypeStatistics(questions: any[]): Record<string, number> {
...
function getBloomsStatistics(questions: any[]): Record<string, number> {
  const stats: Record<string, number> = {};
  questions.forEach(q => {
