...
export const uploadAnswerSheet = async (req, res) => {
    try {
        ...
        const uploadResult = await cloudStorage.uploadAnswerSheet(file.buffer, examId, studentId, file.originalname, file.buffer // Use original buffer for now, can be processed later
        );
...
export const getAnswerSheetsByExam = async (req, res) => {
    try {
...
export const markAsMissing = async (req, res) => {
    try {
...
export const markAsAbsent = async (req, res) => {
    try {
...
export const acknowledgeNotification = async (req, res) => {
    try {
        ...
        await Notification.updateMany({ relatedEntityId: answerSheetId, status: 'UNREAD' }, { status: 'ACKNOWLEDGED', acknowledgedAt: new Date() });
...
export const getAnswerSheetDetails = async (req, res) => {
    try {
        const { answerSheetId } = req.params;
        const userId = req.auth?.sub;
        if (!userId) {
...
export const updateAICorrectionResults = async (req, res) => {
    try {
        const { answerSheetId } = req.params;
...
// Add manual override
export const addManualOverride = async (req, res) => {
    try {
        const { answerSheetId } = req.params;
...
export const batchUploadAnswerSheets = async (req, res) => {
    try {
        const { examId } = req.params;
        const uploadedBy = req.auth?.sub;
        ...
        for (const file of files) {
            try {
                ...
                const processingResult = await ImageProcessingService.processAnswerSheet(file.buffer, file.originalname);
                // Upload to cloud storage
                const uploadResult = await cloudStorage.uploadAnswerSheet(file.buffer, examId, 'unknown', // Will be updated after roll number detection
                file.originalname, processingResult.processedImage || file.buffer);
...
export const processAnswerSheet = async (req, res) => {
    try {
        const { answerSheetId } = req.params;
        const userId = req.auth?.sub;
        if (!userId) {
        ...
        await NotificationService.createAIProcessingStartedNotification(answerSheet.uploadedBy.toString(), answerSheetId, answerSheet.studentId?.name || 'Unknown Student');
        ...
        processAnswerSheetWithAI(answerSheetId, answerSheet)
            ...
            const { NotificationService } = await import('../services/notificationService');
            await NotificationService.createAIProcessingFailedNotification(answerSheet.uploadedBy.toString(), answerSheetId, answerSheet.studentId?.name || 'Unknown Student', error.message);
        });
...
async function processAnswerSheetWithAI(answerSheetId, answerSheet) {
    try {
        ...
        const questionPaper = {
            questions: exam.questionPaperId.questions.map((q) => ({
        ...
        const { NotificationService } = await import('../services/notificationService');
        await NotificationService.createAICorrectionCompleteNotification(answerSheet.uploadedBy.toString(), answerSheetId, answerSheet.studentId?.name || 'Unknown Student', aiResult.percentage, aiResult.confidence);
...
//# sourceMappingURL=answerSheetController.js.map