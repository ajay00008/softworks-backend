...
export async function createExam(req, res, next) {
    try {
        ...
        const invalidSubjects = subjectsWithTemplates.filter(subject => !subject.classIds.some(classId => classId.toString() === examData.classId));
        ...
        const subjectsWithoutReferenceBooks = subjectsWithTemplates.filter(subject => !subject.referenceBook || !subject.referenceBook.fileName);
        ...
        const subjectsWithoutTemplates = subjectsWithTemplates.filter(subject => !subject.templates || subject.templates.length === 0);
...
export async function updateExam(req, res, next) {
    try {
        ...
        const updatedExam = await Exam.findByIdAndUpdate(id, updateData, { new: true, runValidators: true }).populate('subjectIds', 'code name shortName')
            .populate('classId', 'name displayName level section')
            .populate('createdBy', 'name email')
            .populate('questions');
        res.json({
