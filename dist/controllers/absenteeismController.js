...
export async function reportAbsenteeism(req, res, next) {
    try {
        ...
        if (absenteeismData.type === "ABSENT") {
            await Result.findOneAndUpdate({ examId: absenteeismData.examId, studentId: absenteeismData.studentId }, {
                isAbsent: true,
...
export async function getAbsenteeismReports(req, res, next) {
    try {
        ...
        if (acknowledgedBy)
            query.acknowledgedBy = acknowledgedBy;
...
export async function getAbsenteeismReport(req, res, next) {
    try {
        const { id } = req.params;
...
export async function acknowledgeAbsenteeism(req, res, next) {
    try {
        const { id } = req.params;
        ...
        const updatedAbsenteeism = await Absenteeism.findByIdAndUpdate(id, {
            status: "ACKNOWLEDGED",
            acknowledgedBy: userId,
            acknowledgedAt: new Date(),
            adminRemarks
        }, { new: true }).populate('examId', 'title examType scheduledDate')
            .populate('studentId', 'name email')
            .populate('reportedBy', 'name email')
            .populate('acknowledgedBy', 'name email')
            .populate('escalatedTo', 'name email');
...
// Resolve Absenteeism
export async function resolveAbsenteeism(req, res, next) {
    try {
        const { id } = req.params;
        const { adminRemarks } = req.body;
        const userId = req.user.id;
...
export async function escalateAbsenteeism(req, res, next) {
    try {
        const { id } = req.params;
...
// Update Absenteeism
export async function updateAbsenteeism(req, res, next) {
    try {
        const { id } = req.params;
        ...
        if (!absenteeism)
            throw new createHttpError.NotFound("Absenteeism report not found");
        const updatedAbsenteeism = await Absenteeism.findByIdAndUpdate(id, updateData, { new: true, runValidators: true }).populate('examId', 'title examType scheduledDate')
            .populate('studentId', 'name email')
            .populate('reportedBy', 'name email')
            .populate('acknowledgedBy', 'name email')
            .populate('escalatedTo', 'name email');
...
// Delete Absenteeism
export async function deleteAbsenteeism(req, res, next) {
    try {
        const { id } = req.params;
        const absenteeism = await Absenteeism.findById(id);
        if (!absenteeism)
...
export async function getAbsenteeismStatistics(req, res, next) {
    try {
        ...
        if (startDate)
            matchQuery.reportedAt = { $gte: new Date(startDate) };
        if (endDate)
            matchQuery.reportedAt = { ...matchQuery.reportedAt, $lte: new Date(endDate) };
...
//# sourceMappingURL=absenteeismController.js.map