...
export async function getTeachers(req, res, next) {
    try {
        ...
        const transformedTeachers = teachers.map(teacher => ({
            ...
            subjects: teacher.subjectIds.map((subject) => ({
                id: subject._id,
                code: subject.code,
                name: subject.name,
                shortName: subject.shortName,
...
export async function updateTeacher(req, res, next) {
    try {
        ...
        if (updateData.email && updateData.email !== teacher.userId.email) {
        ...
        const transformedTeacher = {
            ...
            email: updatedTeacher.userId.email,
            name: updatedTeacher.userId.name,
            subjects: updatedTeacher.subjectIds.map((subject) => ({
                id: subject._id,
                code: subject.code,
                name: subject.name,
                shortName: subject.shortName,
            ...
            isActive: updatedTeacher.userId.isActive,
            createdAt: updatedTeacher.userId.createdAt,
...
// Activate Teacher
export async function activateTeacher(req, res, next) {
    try {
        const { id } = req.params;
        const teacher = await Teacher.findOne({ userId: id });
        if (!teacher)
...
export async function assignClasses(req, res, next) {
    try {
        const { id } = req.params;
